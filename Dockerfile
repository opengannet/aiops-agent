...
COPY . .
ARG VERSION=unknown
RUN CGO_ENABLED=1 GOPROXY=https://goproxy.cn,direct go build -mod=readonly -ldflags "-extldflags='-Wl,-z,lazy' -X 'github.com/coroot/coroot-node-agent/flags.Version=${VERSION}'" -o coroot-node-agent .
